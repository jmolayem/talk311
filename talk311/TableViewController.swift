...
class TableViewController: PFQueryTableViewController, UISearchBarDelegate {
	...
	required init(coder aDecoder: NSCoder) {
		...
		self.parseClassName = "311talks"
		self.textKey = "Facility"
	...
	override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath, object: PFObject?) -> PFTableViewCell {
		...
		if let Facility = object?["Facility"] as? String {
			cell.customFacility.text = Facility
		}
		if let Email = object?["Email"] as? String {
			cell.customEmail.text = Email
		}
	...
	override func queryForTable() -> PFQuery {
		...
		let query = PFQuery(className: "311talks")
		...
		// Order the results
		query.orderByAscending("Facility")
